...
##' Estimate RNA velocity using gene-relative slopes, the same as velocyto.R package. 
##'
...
##' Estimate Region velocity using steady-state model
##'
...
##' @param RNA_mat - RNA count matrix for theta calculation. If theta value is required, this parameter cannot be optional. 
##' @param old.fit - optional old result (in this case the slopes and offsets won't be recalculated, and the same kNN graphs will be used)
##' @param mult - library scaling factor (1e6 in case of FPM)
##' @param min.imat.smat.correlation - minimum required Spearman rank correlation between introns and spanning read counts of a gene
##' @param min.imat.emat.correlation - minimum required Spearman rank correlation between introns and exons counts of a gene
##' @param min.imat.emat.slope - minimum sloope of i~e regression
##' @param zero.offset - should offset be set to zero, or determined (through smat regression or using near-0 e cases)
##' @param deltaT2 - scaling of the projected difference vector (normally should be set to 1)
##' @param fit.quantile perform gamma fit on a top/bottom quantiles of expression magnitudes
##' @param diagonal.quantiles whether extreme quantiles should be computed diagonally
...
##' @param do.par whether the graphical device parameters should be reset as part of show.gene (default=TRUE)
##' @param cell.dist - cell distance to use in cell kNN pooling calculations
##' @param emat.size - pre-calculated cell sizes for the emat (exons) matrix
##' @param imat.size - pre-calculated cell sizes for the imat (introns) matrix
##' @param cell.emb - cell embedding to be used in show.gene function
##' @param cell.colors - cell colors to be used in show.gene function
##' @param expression.gradient - color palette used to show the expression magnitudes in show.gene function
##' @param residual.gradient - color palette used to show the u residuals in show.gene function
##' @param n.cores - number of cores to use
##' @param verbose - output messages about progress
##' @return a list with velocity results, including the current normalized expression state ($current), projected ($projected) over a certain time ($deltaT), cellsize of emat ($cellsize), unscaled transcriptional change ($deltaE), fit results ($gamma, $ko, $sfit if spanning reads were used), optional cell pooling parameters ($cellKNN, $kCells), kNN-convolved normalized matrices (conv.imat.norm and conv.emat.norm), library scale ($mult)
##' @examples
##' \dontrun{
##'  # use min/max quantile gamma fit (recommended option when one can afford to do cell kNN smoothing)
##'  # The example below uses k=10 cell kNN pooling, and top/bottom 5% exprssion quantiles using 8 cores
##'  # cell_e_gexp and cell_i_gexp are exons and introns count matirces and cell_RNA_gexp is RNA expression matrix
##' (preferably filtered for informative genes)
##'  rvel <- gene.region.velocity.estimates(cell_e_gexp,cell_i_gexp,theta.s = T, RNA_mat = cell_RNA_gexp,
##'                                         n.cores = 8,kCells = 10,fit.quantile = 0.05)
##'
##'  # alternativly, the function can be used to visualize gamma fit and regression for a
##' particular gene. here we pass embedding (a matrix/data frame with rows named with cell names,
##' and columns corresponding to the x/y coordinates)
##' 
##'  # and cell colors. old.fit is used to save calculation time.
##'  gene.region.velocity.estimates(cell_e_gexp,cell_i_gexp,kCells = 10,fit.quantile = 0.05,
##'     old.fit=rvel,show.gene='Ldhc',cell.emb=emb,cell.colors=cell.colors)
##' }
##' @export
gene.region.velocity.estimates <- function(emat,imat,deltaT=1,smat=NULL,steady.state.cells=colnames(emat),kCells=10,cellKNN=NULL,kGenes=1,theta.s=FALSE, RNA_mat=NULL, old.fit=NULL,mult=1e3,min.imat.smat.correlation=0.05,min.imat.emat.correlation=0.05, min.imat.emat.slope=0.05, zero.offset=FALSE,deltaT2=1, fit.quantile=NULL, diagonal.quantiles=FALSE, show.gene=NULL, do.par=TRUE, cell.dist=NULL, emat.size=NULL, imat.size=NULL, cell.emb=NULL, cell.colors=NULL, expression.gradient=NULL,residual.gradient=NULL, n.cores=defaultNCores(), verbose=TRUE) {
  if(!all(colnames(emat)==colnames(imat))) stop("emat and imat must have the same columns (cells)");
  if(!is.null(smat)) { if(!all(colnames(emat)==colnames(smat))) stop("smat must have the same columns (cells) as emat") }
  resl <- list();
  vg <- intersect(rownames(emat),rownames(imat)); #intersect same genes
  if(is.null(smat)) {
  ...
  conv.emat.norm <- t(t(conv.emat)/conv.emat.cs) #normalization
  conv.imat.norm <- t(t(conv.imat)/conv.imat.cs)
  emat.norm <- t(t(emat)/emat.cs)
  ...
  if(is.null(old.fit)) {
    ko <- data.frame(do.call(rbind,parallel::mclapply(sn(rownames(conv.emat.norm)),function(gn) {
      beta=1 #default value of beta, it won't influence the extrapolated cell state but will influence the initial value for EM algorithm in dyanamical model.
      if(!is.null(smat)) {
        df <- data.frame(i=(conv.imat.norm[gn,steady.state.cells]),e=(conv.emat.norm[gn,steady.state.cells]),o=sfit[gn,'o'])
        if(zero.offset) df$o <- 0;
      }  else {
      ...
      if(theta.s) {
        #emat and imat are exons number of a gene plus gene counts. Expression matrix is used to remove influnce of gene counts to calculate theta. 
        ...
        theta=0.5 #default exons:introns=1:1
      }
      if(is.null(fit.quantile)) {
        d <- lm(i~e+offset(o)+0,data=df,weights=df$e^4+df$i^4);
        ...
        if(!is.null(smat) || zero.offset) {
          ...
          alpha=mean(beta/theta*imat.lm) # alpha value won't be used in steady-state model but will used for initialization in EM algorithm of dynamical model.
  ...
  cat("calculating Region velocity shift ... ")
  if(kGenes>1) {
...
##' Estimate Region velocity using dynamical model with EM algorithm
##'
##' @param vel - velocity estimation(from gene.region.velocity.estimates)
##' @param iteration - Max steps of EM iterations
##' @param lower - Lower value of parameters(alpha,beta,gama). It must be larger than 0. 
##' @param deltaT - amount of time to project the cell forward
##' @param n.cores - number of cores to use
##' @param verbose - output messages about progress
##' @return a list with velocity results, including the current normalized expression state ($current), projected ($projected) over a certain time ($deltaT), cellsize of emat ($cellsize), unscaled transcriptional change ($deltaE), fit results ($gamma, $ko, $sfit if spanning reads were used), optional cell pooling parameters ($cellKNN, $kCells), kNN-convolved normalized matrices (conv.imat.norm and conv.emat.norm), library scale ($mult), time ($time) and inferred model parameters ($para.new)
##' @examples
##' \dontrun{
##'  # use EM algorithm to infer parameters of analytical solution.
##'  # rvel is the velocity result of steay-state. (from gene.region.velocity.estimates)
##'  EM_rvel <-  gene.EM.velocity.estimates(rvel,n.cores = 8)
##' }
##' @export
gene.EM.velocity.estimates <- function(vel,iteration=100, lower=1e-6,deltaT=1,n.cores=defaultNCores(), verbose=TRUE) {
  ...
  vel$time <- para_new_mat[,6:ncol(para_new_mat)]
  vel$para.new <- para_new_mat[,1:5]
...
##' Joint t-SNE visualization of the velocities by joint t-SNE embedding of both current and extraploated cell positions, same as velocyto.R package
##'
...
##' Visualize RNA velocities on an existing embedding using correlation-based transition probability matrix within the kNN graph, same as velocyto.R package
##'
...
##' Visualize RNA velocities on an existing embedding using Euclidean-based transition probability matrix within the kNN graph,same as velocyto.R package.
##'
...
##' DEPRECATED: Read in cell-specific bam files for SMART-seq2 measurement, same as velocyto.R package
...
##' Read in cell-specific bam files for STRT/C1, same as velocyto.R package
##'
...
# estimate expression of a projected cell given the original exons matrix and deltaE matrix
...
# estimate expression of a projected cell given the original exons matrix and deltaE matrix using EM algorithm
...
# reads layers/spliced/unspliced/ambiguous from a loom file, same as velocyto.R package
...
##' identify positions of likely internal priming sites by looking for polyA/polyT stretches within, same as velocyto.R package
...
##' read in detailed molecular mapping info from hdf5 file as written out by "-d" option of velocyto.py, same as velocyto.R package
##'
...
single.gene.EM.inference <- function(emat,imat,para,theta,lower=1e-6,ITR=100){
  ...
  npara <- c(alpha,beta,gama,ts=ts,ITR=max.itr+1-itr)
  rv <- c(npara,t)
  return(rv)
}
